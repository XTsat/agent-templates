...
**预设 B：三件套同步式**（i18n 项目，changelog 在页面中渲染）

如果 changelog 是在页面中渲染的（如 `src/app/[locale]/changelog/page.tsx`），则 changelog 变更需要 **三处同步**：

| # | 文件 | 操作 |
|---|------|------|
| 1 | `src/app/[locale]/changelog/page.tsx` | 在 `entries` 数组**最上方**新增一条记录 |
| 2 | `messages/zh-CN.json` | 新增对应文案键 |
| 3 | `messages/en.json` | 新增同键名的英文条目 |
