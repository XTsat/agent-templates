...
AstrBot 插件生态中有三种既有格式，**新建项目时选择一种，在整个生命周期内保持一致**：

---

**预设 A：扁平前缀式**（参考 `astrbot_plugin_msg_forward_cc`）
...
**预设 B：分类节式**（参考 `astrbot_plugin_model_status`）
...
**预设 C：扁平分类式**（参考 `astrbot_plugin_help_panel_typst`）
...
前缀分类与预设 A 相同，但顺序为 `[Unreleased]` → `## vX.Y.Z (YYYY-MM-DD)`，且分类前缀内联在条目开头。
